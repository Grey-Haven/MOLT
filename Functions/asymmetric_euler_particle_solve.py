...
from map_fields import map_J_to_mesh_2D2V, map_rho_to_mesh_2D, charge_conserving_rho_update
...
def asym_euler_particle_solver(x1_ions_in, x2_ions_in,
    ...
    while(steps < N_steps):
        ...
        charge_conserving_rho_update(rho_mesh, J_mesh, x, y, dx, dy, dt, x1_elec_new[:N_elec_now], x2_elec_new[:N_elec_now])
        ...
        if enable_csvs and (steps % plot_at == 0 or steps + 1 == N_steps):
            ...
            particle_information = np.column_stack([x1_elec_old[:N_elec_now], x2_elec_old[:N_elec_now], v1_elec_old[:N_elec_now], v2_elec_old[:N_elec_now]])
            print(electron_csv_path+"electron"+step_pad+csv)

...
void MOLTEngine::computeTotalEnergy() {
    ...
    // std::cout << eleTotalEnergy << std::endl;
...
void MOLTEngine::computeGaugeL2() {
    ...
    for (int i = 0; i < Nx*Ny; i++) {
        if (this->method == MOLTEngine::DIRK2 || this->method == MOLTEngine::DIRK3) {
            ...
            if (this->method == MOLTEngine::BDF1 || this->method == MOLTEngine::CDF1 || this->method == MOLTEngine::MOLT_BDF1) {
...
void MOLTEngine::computeGaussL2() {
    ...
    if (this->method == MOLTEngine::DIRK2) {
        ...
        for (int i = 0; i < Nx*Ny; i++) {
            if (this->method == MOLTEngine::BDF1 || this->method == MOLTEngine::MOLT_BDF1 || this->method == MOLTEngine::DIRK2) {
...
void MOLTEngine::scatterFields() {
    ...
    // }
    // std::cout << "alpha: " << std::setprecision(16) << alpha << std::endl;
    // std::cout << "rhoTotalPrev: " << std::setprecision(16) << rhoTotalPrev << std::endl;
    // std::cout << "rhoTotal: " << std::setprecision(16) << rhoTotal << std::endl;
    // std::cout << "===============================" << std::endl;
...
void MOLTEngine::get_L_x_inverse_per(std::complex<double>* u, std::complex<double>* inverseOut) {
    ...
    for (int j = 0; j < Ny; j++) {
        ...
        linear5_L(u_ext, alpha, J_L);
        linear5_R(u_ext, alpha, J_R);
        ...
        apply_A_and_B(J, x, Nx, alpha, A_x, B_x);
...
void MOLTEngine::get_ddx_L_x_inverse_per(std::complex<double>* u, std::complex<double>* ddxOut) {
    ...
    for (int j = 0; j < Ny; j++) {
        ...
        for (int i = 0; i < J_N; i++) {
            ...
            int idx = i*Ny + j;
            ddxOut[idx] = -alpha*J[i] - alpha*A_x*mu_i1 + alpha*B_x*mu_i2;
        }
    }
}
...
void MOLTEngine::get_ddy_L_y_inverse_per(std::complex<double>* u, std::complex<double>* ddyOut) {
    ...
    for (int i = 0; i < Nx; i++) {
        ...
        for (int j = 0; j < Ny; j++) {
            ...
            int idx = i*Ny + j;
            ddyOut[idx] = -alpha*J[j] - alpha*A_y*mu_j1 + alpha*B_y*mu_j2;
        }
    }
...
void MOLTEngine::apply_A_and_B(std::vector<std::complex<double>> &I_, double* x, int N, double alpha, double A, double B) {

    int I_len = I_.size();

    for (int i = 1; i < I_len; i++) {
        I_[i] += A*std::exp(-alpha*( x[i  ] - x[0] ));
        I_[i] += B*std::exp(-alpha*( x[N-1] - x[i] + dx ));
    }
}

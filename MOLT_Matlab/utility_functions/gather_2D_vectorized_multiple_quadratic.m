function F_ps = gather_2D_vectorized_multiple_quadratic(F_meshes, x1_p, x2_p, x, y, dx, dy)
    %%%%%%%%
    ...
    try
        ...
        idx1 = sub2ind(size(F_meshes(:,:,1)), y_idx_m1, x_idx_m1);
        idx2 = sub2ind(size(F_meshes(:,:,1)), y_idx_m1, x_idx   );
        idx3 = sub2ind(size(F_meshes(:,:,1)), y_idx_m1, x_idx_p1);
    
        idx4 = sub2ind(size(F_meshes(:,:,1)), y_idx   , x_idx_m1);
        idx5 = sub2ind(size(F_meshes(:,:,1)), y_idx   , x_idx   );
        idx6 = sub2ind(size(F_meshes(:,:,1)), y_idx   , x_idx_p1);
    
        idx7 = sub2ind(size(F_meshes(:,:,1)), y_idx_p1, x_idx_m1);
        idx8 = sub2ind(size(F_meshes(:,:,1)), y_idx_p1, x_idx   );
        idx9 = sub2ind(size(F_meshes(:,:,1)), y_idx_p1, x_idx_p1);

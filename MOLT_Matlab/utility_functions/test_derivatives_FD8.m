...
for g = grids
    ...
    u_ddx_FD8 = compute_ddx_FD8_per(u(1:end-1,1:end-1), dx);
    u_ddy_FD8 = compute_ddy_FD8_per(u(1:end-1,1:end-1), dy);

    errs_x(i) = max(max(abs(u_ddx(1:end-1,1:end-1) - u_ddx_FD8)));
    errs_y(i) = max(max(abs(u_ddy(1:end-1,1:end-1) - u_ddy_FD8)));
    dxs(i) = dx;
    dys(i) = dy;
    i = i+1;
end
...
loglog(dxs,dxs.^5);
loglog(dxs,dxs.^6);
legend(["Errors x", "Errors y", "Linear", "Quadratic", "Cubic", "Quartic", "Quintic", "Sextic"])
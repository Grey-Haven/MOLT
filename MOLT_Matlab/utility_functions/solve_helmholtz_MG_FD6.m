function u_fine = solve_helmholtz_MG_FD6(u_guess, RHS, alpha, dx, dy)
    ...
    for k = 1:MAX
        ...
        u_fine = solve_helmholtz_GS_FD6(u_fine, RHS, alpha, dx, dy, 50, 1e-1);

        % Compute residual
        laplacian_u_num = compute_Laplacian_FD6(u_fine, dx, dy);
        ...
        e_H = solve_helmholtz_GS_FD6(u_coarse_guess, res_coarse, alpha, dx, dy, MAX, TOL);
        ...
        % if (max(max(abs(u_prev - u_fine))) < TOL)

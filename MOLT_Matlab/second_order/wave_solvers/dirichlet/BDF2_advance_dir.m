function v = BDF2_advance_dir(v, src_data, x, y, dx, dy, dt, c, beta)
    %%%
    ...
    R = .5 * ( 5*v(:,:,end) - 4*v(:,:,end-1) + v(:,:,end-2) + ( 1/(alpha^2) )*src_data );
    ...
    v(:,:,end) = get_L_x_inverse_dir(tmp, x, y, dx, dy, dt, c, beta);
